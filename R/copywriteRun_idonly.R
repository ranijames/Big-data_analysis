...
sample.control = data.frame(samples=c(input_file_tumour, input_file_normal), 
                            controls=c(input_file_normal, input_file_normal))
...
colnames(log2_df)[5:6] <- c(p.id, p.cr)
print(colnames(log2_df))
...
wanted_out_cols = c('Chromosome', 'Start', 'End', 'Gene', p.id, p.cr, id.vs.cr)
